...
This is a Java 25 + Spring Boot 4 message board application with a React 19 + Vite 7 frontend. See `README.md` for full details.

Backend Java packages under `org.bruneel.thankyouboard`: `config`, `domain` (entities), `repository`, `service`, `web` (REST controllers), `web.support` (API version handler, trace filter), `security`, `validation`, `observability`, `images` (upload/presign), plus `ThankYouBoardApplication` at the root. The `functions/` module keeps its own `handler` / `model` / `service` layout for Lambda.
...
- **Backend tests:** `mvn test` — uses H2 in-memory DB, no external services needed.
- **Frontend lint:** `cd frontend && npm run lint` — has 2 pre-existing lint errors (not introduced by setup).
- **Frontend build:** `cd frontend && npm run build` — has a pre-existing TypeScript type error in `AddPostModal.tsx` (Giphy type mismatch). The Vite dev server still works since it doesn't enforce strict TS compilation.
...
- The backend OTLP/Grafana exporter will log repeated WARN lines about failed export (HTTP 401) unless `grafana.otlp.headers` is configured. This is harmless.
- Giphy endpoints return 503 when `GIPHY_API_KEY` is not set. The rest of the app works normally.
