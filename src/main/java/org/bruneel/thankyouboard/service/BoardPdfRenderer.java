...
public final class BoardPdfRenderer {
    ...
    /** Human-readable date in UTC when no display timezone is set. */
    ...
    /** Human-readable date in a specific zone (e.g. "6:31 AM EST"). */
    ...
    public static byte[] render(Board board, List<Post> posts) {
        return render(board, posts, RenderConfig.defaults(), null);
    }

    public static byte[] render(Board board, List<Post> posts, RenderConfig renderConfig) {
        return render(board, posts, renderConfig, null);
    }

    /**
     * Renders the board PDF. When {@code displayTimeZone} is null, the "Created" date is shown in UTC
     * with the label "UTC". When provided (e.g. user's browser zone), the date is shown in that zone
     * with its short name (e.g. "EST", "CET") for a friendlier experience.
     */
    public static byte[] render(Board board, List<Post> posts, RenderConfig renderConfig, ZoneId displayTimeZone) {
        try (PDDocument document = new PDDocument();
            ...
            canvas.drawHeader(board, displayTimeZone);
    ...
    private static PDFont loadFallbackFont(PDDocument document) throws IOException {
        try (InputStream fallbackStream = BoardPdfRenderer.class.getResourceAsStream(FALLBACK_FONT_RESOURCE)) {
            return fallbackStream != null ? PDType0Font.load(document, fallbackStream) : null;
        }
    }
    ...
    public static String toFilename(Board board) {
    ...
    private static final class PdfCanvas {
        ...
        private void drawHeader(Board board, ZoneId displayTimeZone) throws IOException {
            ...
            if (board.getCreatedAt() != null) {
                String dateText = displayTimeZone == null
                        ? DATE_FORMAT_UTC.format(board.getCreatedAt())
                        : DATE_FORMAT_WITH_ZONE.withZone(displayTimeZone).format(board.getCreatedAt());
